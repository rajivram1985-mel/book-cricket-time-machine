...
`voice:generate` runs `scripts/generate-voice-clips.ts` via `vite-node` (already a transitive dep of vitest — no new package added) so it can import `src/voiceLines.ts` and `src/roster.ts` directly as the single source of truth. It reads `ELEVENLABS_API_KEY` (required) and optional `ELEVENLABS_VOICE_ID`/`ELEVENLABS_MODEL_ID` from a hand-rolled `.env.local` loader (no dotenv dependency; the file is covered by the existing `*.local` gitignore pattern — never paste the key into chat or commit it). It's idempotent — reruns skip files that already exist — and writes into `public/audio/voice/`. Those MP3s are committed like any other static asset (not gitignored): that keeps a git-integrated static host (Vercel/Netlify) able to build and deploy without ever needing the ElevenLabs key as a CI secret.
...
- **`src/voiceLines.ts` / `src/voice.ts`** — pre-generated commentary, voiced on wickets, fours, sixes, power plays and verdicts only (never singles/twos/threes/fives — see `resolveBallMoment`'s doc comment). `voiceLines.ts` is name-free script text (dynamic names stay in `commentary.ts`'s on-screen text); `voice.ts` has two layers: pure resolvers (`resolveBallMoment`, `resolveMatchMoment`, unit-tested, and deliberately mirror `commentaryFor`'s priority order — a `tests/voice.test.ts` case cross-checks the two never disagree) and untested browser playback (`playMomentVoice`/`playNameCallout`) that fails silently on a 404. **The game must be fully playable with zero audio files present** — clips are static MP3s generated by `scripts/generate-voice-clips.ts` (`npm run voice:generate`, needs `ELEVENLABS_API_KEY` in a gitignored `.env.local`; never call this API from the shipped app itself, or the key would leak to every player).
- **`src/avatar.ts`** — generated inline-SVG avatars (initials + emoji + country gradient); no photo assets.

Tests (`tests/engine.test.ts`, `tests/daily.test.ts`, `tests/storage.test.ts`, `tests/voice.test.ts`, vitest) cover only the pure layer — engine math, commentary non-repetition, daily determinism, storage recorders, and voice-moment resolution. Keep new game logic in `engine.ts`/`daily.ts`/`voice.ts`'s pure half so it stays testable; `main.ts` (and `voice.ts`'s playback half) is untested by design.
