...
- **`src/audio.ts`** — synthesized Web Audio sound cues (flip/runs/boundary/wicket); no audio assets. The sound toggle is in-memory state only — nothing may be written to localStorage, or the footer's "nothing is stored" promise breaks.
